...
class TestSkeleton
  ...
  def remove_array_elements(source_array, values_array)
    source_array.reject { |el| values_array.include?(el) }
  end
  ...
  def middle_chars(test_string)
    test_string.length.even? ? test_string[test_string.length/2-1,2] : test_string[test_string.length/2.to_i]
  end
end
