...
class TestSkeleton
  ...
  def my_languages(hash)
    hash.select {|key, value| value >= 60 }.sort_by { |key, val| -val }.to_h.keys
  end
  ...
  def consecutive_duplicates(string)
    ...
    string.split.inject do |memo, word|
      str = prev = memo  if prev.empty?   
      if(prev != word)
        str += " " + word
        prev = word
      end   
  ...
  def middle_chars(test_string)
    ...
    if(test_string.size.even?)
      test_string.slice((test_string.size/2) - 1, 2)   
    else
      test_string.slice((test_string.size/2), 1)
    end 
